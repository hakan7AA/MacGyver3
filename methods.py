...
def generate_laby(player, exit):
    ...
    with open('macgyverlaby.txt') as laby: 
        str_laby = ''.join(str(line) for line in laby) #expression generatrice (pour les element de ma list '', les element iterable(element dont on peut parcourir les valeurs) c'est join ,  - ici les lignes ), pour parcourir chaque element de la ligne c'est str(line) for ...
        list_str = list(str_laby) #on dit que chaque ligne c'est une liste 
        laby = [] # c'est le labyrinthe, la grande liste generale
        x = 0
        y = 0
        ...
        if square.get_coord().get_x() == item.get_coord().get_x() and square.get_coord().get_y() == item.get_coord().get_y(): 
            if (item.get_coord().get_x() != exit.get_coord().get_x() and item.get_coord().get_y() != exit.get_coord().get_y()) and (item.get_coord().get_x() != player.get_coord().get_x() and item.get_coord().get_y() != player.get_coord().get_y()):
                if square.get_has_item() is True: #  item(option oui)
                    ...
                    break #pour executer deja sa                     
                if square.get_is_wall() is True: #mur (option oui)
                ...
                if square.get_is_wall() is False and square.get_has_item() is False:#si ya pas de mur ni d'item alors 
                    ...
                    break #pour sortir de la boucle atout moment    
